...
(function (us) {
  ...
  const width = 975;
  const height = 610;
  const path = index(); // const jsonUrl = 'https://unpkg.com/world-atlas@2.0.2/countries-50m.json';

  const svg = create("svg").attr("height", height).attr("width", width);
  svg.append("path").attr("fill", "#ddd") // .attr("d", d3.geo.path().projection(d3.geo.mercator()));
  .attr("d", path(feature(us__default['default'], us__default['default'].objects.nation)));
  svg.append("path").attr("fill", "none").attr("stroke", "#fff").attr("stroke-linejoin", "round").attr("stroke-linecap", "round");
